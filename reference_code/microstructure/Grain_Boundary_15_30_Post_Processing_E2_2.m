...
for idx = 1:iter_range
    ...
    % figure;
    % [ax, h1, h2] = plotyy(groups_depth_time*1e3,norm_var1,groups_depth_time*1e3,norm_var2);
    % set(h1,'LineWidth', lw);
    % set(h2,'LineWidth', lw);
    % set(gca,'FontName','Times','FontSize',12);
    % xlabel('Depth (mm)','Interpreter','latex')
...
% 
    ...
    figure;
    plot(groups_depth_time*1e3, norm_var1, 'LineWidth', 1) %, 'Color', [0.9290, 0.6940, 0.1250] );
    hold on;
    plot(groups_depth_time*1e3, norm_var2, 'LineWidth', 1.5) %, 'Color', [0.4940, 0.1840, 0.5560]);
    hold on;
    set(gca,'FontName','Times','FontSize',14);
    xlabel('Depth (mm)','Interpreter','latex')
    ylabel('Spatial variance (Normalised)', 'Interpreter','latex')
    xline(depth_at_max_2, '--k', 'LineWidth', 1.2);
    legend('M1 Spatial Variance', 'RMLE filter result', 'Interpreter', 'latex','FontSize',14)
    ylim([0,1.25])
    filename = 'I1_RMLE.png';
    print(filename, '-dpng', '-r300');
    %fprintf('Peak of second dataset: %f at depth %f mm\n', max_value_2, depth_at_max_2);
    %fprintf(depth_at_max_2)
    fprintf('%f\n', depth_at_max_2);

    % Determine the minimum length between groups_depth_time and the number of rows in igroups_data_scatter
    % min_length = min(length(groups_depth_time), size(igroups_data_scatter, 1));
    % 
    % % Iterate over each column in igroups_data_scatter
    % for i = 1:size(igroups_data_scatter, 2)
    %     % Plot current column of igroups_data_scatter against groups_depth_time
    %     plot(groups_depth_time(1:min_length), igroups_data_scatter(1:min_length, i));
    %     hold on; % This will hold the plot so that subsequent plots are overlaid
    % end
    % 
    % % Add labels and legend
    % xlabel('Depth Time');
    % ylabel('Data Values');
    % title('Data vs. Depth Time');
    % 
    % Add a legend showing which column corresponds to which data
    %legend('Data 1', 'Data 2', 'Data 3', ...); % Add more legend entries as needed
