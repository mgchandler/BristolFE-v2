...
RUN_NUM = [1];
sample_width = 1e-3/4; % /4
sample_depth = 4e-3/4; %optimum 3.5 /4
sample_depth_1 = 1.5e-3/4; %optimum 1.3 /4
...
for run_num=1:RUN_NUM(ig1)
    ...
    [VX_1,VY_1,V_1,C_1,VX_2,VY_2,V_2,C_2]=fn_grain_structure_two_grain_size_4...
    ...
    box on;
    set(gca,'FontName','Times','FontSize',12);
    legend(['GS1: $  \ \sigma_1 =' num2str(round(std_1,1)) '\mathrm{\mu m}$' ],...
        ['GS2: $ \ \sigma_2 =' num2str(round(std_2,1)) '\mathrm{\mu m}$' ],'location','northeast','Interpreter','latex','FontSize', 12);
    % title(['$g1-' num2str(g1*1E6) '\ g2-' num2str(g2*1E6) '\ ' num2str(round(sample_width*1e3))...
...
%     num_unique_values = length(unique(I2(:)));
% 
...
% 
%     % Randomly shuffle the rows of cmap (optional)
