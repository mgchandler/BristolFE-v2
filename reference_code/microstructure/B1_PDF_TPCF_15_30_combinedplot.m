...
for run_num=1:RUN_NUM(ig1)
    ...
    data_S1.r_1_S1 = r_1_S1;
    data_S1.wr_1_S1 = wr_1_S1;
    data_S1.wr_2_S1 = wr_2_S1;
    data_S1.r_2_S1 = r_2_S1;
    data_S1.ad_1_S1 = ad_1_S1;
    data_S1.aL_1_S1 = aL_1_S1;
    data_S1.aL_2_S1 = aL_2_S1;
    data_S1.ad_2_S1 = ad_2_S1;
    data_S1.std_1_S1 = std_1_S1;
    data_S1.std_2_S1 = std_2_S1;
    data_S1.grain_size_1_S1 = grain_size_1_S1;
    data_S1.grain_size_2_S1 = grain_size_2_S1;
    ...
    % Plot the data
    plot(r_1*pixels_size*1e6, wr_1_S1, 'o-', 'LineWidth', 1); hold on; % Plot with circle markers for wr_1_S1
    plot(r_2*pixels_size*1e6, wr_2_S1, 'o-', 'LineWidth', 1); hold on; % Plot with circle markers for wr_2_S1
    plot(r_1*pixels_size*1e6, wr_1, '^-', 'LineWidth', 1); hold on; % Plot with triangle markers for wr_1
    plot(r_2*pixels_size*1e6, wr_2, '^-', 'LineWidth', 1); % Plot with triangle markers for wr_2

    % % Plot the data
    % plot(r_1*pixels_size*1e6, wr_1,'Linewidth',2, 'Color', [0, 0.25, 0.5]); hold on;
    % plot(r_2*pixels_size*1e6, wr_2,'Linewidth',2, 'Color',[0.6, 0.6, 0.6]);hold on;
    % plot(r_1*pixels_size*1e6, wr_1_S1,'Linewidth',2); hold on;
    % plot(r_2*pixels_size*1e6, wr_2_S1,'Linewidth',2); 
    ...
    legend(["M1\_G1: $\mu = " + num2str(round(ad_1_S1, 1)) +  "\mu m, L_c = " + num2str(round(aL_1*1E6, 1)) + "\mu m$", ...
        "M1\_G2: $\mu = " + num2str(round(ad_2_S1, 1)) + "\mu m, L_c = " + num2str(round(aL_2*1E6, 1)) + "\mu m$", ...
        "M2\_G1: $\mu = " + num2str(round(ad_1, 1)) + "\mu m, L_c = " + num2str(round(aL_1_S1*1E6, 1)) + "\mu m$", ...
        "M2\_G2: $\mu = " + num2str(round(ad_2, 1)) + "\mu m, L_c = " + num2str(round(aL_2_S1*1E6, 1)) + "\mu m$"], ...
       'location','northeast', 'Interpreter', 'latex', 'FontSize', 14);

     % legend(["G1\_I5: $\mu = " + num2str(round(ad_1_S1, 1)) + "\mu m, L_c = " + num2str(round(aL_1_S1*1E6, 1)) + "\mu m$", ...
     %    "G2\_I5: $\mu = " + num2str(round(ad_2_S1, 1)) + "\mu m, L_c = " + num2str(round(aL_2_S1*1E6, 1)) + "\mu m$", ...
     %    "G1\_I1: $\mu = " + num2str(round(ad_1, 1)) + "\mu m, L_c = " + num2str(round(aL_1*1E6, 1)) + "\mu m$", ...
     %    "G2\_I1: $\mu = " + num2str(round(ad_2, 1)) + "\mu m, L_c = " + num2str(round(aL_2*1E6, 1)) + "\mu m$"], ...
    ...
    % % Save the figure
    saveas(fig3, 'TPCF_long.png');

    % filename = 'Comb_TPCF_3.png';
    % print(filename, '-dpng', '-r300');
    %% Plot grain size distribution
    
    % Generate x-values for plotting the bell curve
    x_min = min([min(grain_size_1), min(grain_size_2)]) * 1e6;
    x_max = max([max(grain_size_1) + 15e-6, max(grain_size_2) + 15e-6]) * 1e6;
    ...
    bell_curve3 = normpdf(x_values, ad_1_S1, std_1_S1);
    bell_curve4 = normpdf(x_values, ad_2_S1, std_2_S1);
    ...
    % hold on
    % histogram(grain_size_1_S1*1e6,'binwidth',1,'Normalization', 'pdf'); hold on; %'FaceColor', [0, 0.2, 0.5]
    % hold on
    % histogram(grain_size_2_S1*1e6,'binwidth',1,'Normalization', 'pdf'); %'FaceColor',[0.5, 0.5, 0.5]
    % 
    fwave = figure('Visible','on');
    histogram(grain_size_1_S1*1e6,'binwidth',1,'Normalization', 'pdf', 'FaceColor', [0, 0.25, 0.5]); hold on; %'FaceColor', [0, 0.2, 0.5]
    hold on
    histogram(grain_size_2_S1*1e6,'binwidth',1,'Normalization', 'pdf', 'FaceColor',[0.7, 0.7, 0.7]); %'FaceColor',[0.5, 0.5, 0.5]
    hold on
    ...
    % Generate histograms
    [counts1, edges1] = histcounts(grain_size_1_S1*1e6,'binwidth',1,'Normalization', 'pdf');
    [counts2, edges2] = histcounts(grain_size_2_S1*1e6,'binwidth',1,'Normalization', 'pdf');
    
    % Make sure counts1 and counts2 have the same length
    min_len = min(length(counts1), length(counts2));
    counts1 = counts1(1:min_len);
    counts2 = counts2(1:min_len);
    ...
    % Calculate overlap
    overlap_heights = min([counts1; counts2]);
    overlap_area = sum(overlap_heights) * diff(edges1(1:2));  % Total area of overlap

    % Calculate percentage overlap
    total_area = sum(counts1) * diff(edges1(1:2));  % Total area of histogram 1
    percentage_overlap = overlap_area / total_area * 100;  % Percentage overlap
    
    figure;
    plot(x_values, bell_curve1, 'LineWidth', 1.25); hold on;
    plot(x_values, bell_curve2, 'LineWidth', 1.25); hold on;
    plot(x_values, bell_curve3, 'LineWidth', 1.25); hold on;
    plot(x_values, bell_curve4, 'LineWidth', 1.25);
   ...
   % legend(['G1\_I5: $ \sigma = ' num2str(round(std_1, 1)) '\mu m$'], ...
   %     ['G2\_I5: $ \sigma = ' num2str(round(std_2, 1)) '\mu m$'], ...
   %     ['G1\_I1: $ \sigma = ' num2str(round(std_1_S1, 1)) '\mu m$'], ...
   %     ['G2\_I1: $ \sigma = ' num2str(round(std_2_S1, 1)) '\mu m$'], ...
   %     'location', 'northeast', 'Interpreter', 'latex', 'FontSize', 12);

    

   % filename = 'Comb_PDF_3.png';
   %  print(filename, '-dpng', '-r300');
    %% Plot Image
%     I2 = I1;
% for i=1:max(max(I1))
