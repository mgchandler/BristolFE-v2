...
% 5 els per wavelength:8-10 micrometre 5 MINUTES PER ITERATION
% 10 els per wavelength:4-5 micrometre 35 MINUTE PER ITERATION
% 15 els per wavelength:2.8-3.3 micrometre 1.5-2.5 hrs
% 20 els per wavelength: 2.1-2.3 micrometre 5-8 hrs (14000+4000 secs)

%21/03
% several models obsolete: thin sample doesnt produce wanted output signal.
% what to do: run atleast 5 versions of the reduced model with smaller
...
for iter = 1:length(Iterations)
    ...
    % saveas(gcf, 'Grains_15_30_reduced.fig')
    
    %--------------------------------------------------------------------------
    %RUN THE MODEL
    
    fe_options.field_output_every_n_frames = 10000;
    ...
    % figure;
    % plot(sig.steps_load_time, sig.sum_res_dsps); %see dsps
    % xlabel('Time (s)')
    % 
    ...
    signal_filename = sprintf('%02d_Signal_I5_G%d_G%d_F%d_E%d.mat', Iteration, round(G1*1e6), round(G2*1e6), round(centre_freq/1e6), round(el_size*1e6));
    ...
    folder_name = 'Grain_Boundary_Outputs/E2_2';
